...
class Modurt42Plugin(b3.plugin.Plugin):
    ...
    def cmd_giveweapon(self, data, client, cmd=None):
        ...
        if not sclient:
            ...
            if len(input) == 2:
                ...
                shells = input[2]
            if not weapon:
                client.message('Correct usage is ^2!gw ^4<weapon> <amount>')
                return False
    ...
    def cmd_teleport(self, data, client, cmd=None):
        """\
        - [player] (player2). Teleport to a player or teleport players
        """
        input = self._adminPlugin.parseUserCmd(data)
        if not data:
    ...
    def cmd_load(self, data, client, cmd=None):
        """\
        - [configFile] (1/0) load a config file, use ^2!load ^4[configFile] 1^7 to use the z3n_config cvar
        """
        input = self._adminPlugin.parseUserCmd(data)
        if not data:
            client.message('Use ^2!help ^4load')
            return False
        
        config = input[0]
        z3n_config = input[1]
        if not z3n_config:
            self.console.write('load "%s"' % (config))
            client.message("Config file ^4%s^7 loaded" % config)
        elif z3n_config == "1":
            self.console.write('load "%s"' % (config))
            self.console.write('z3n_config "%s"' % (config))
            client.message("Config file ^4%s^7 loaded and set as z3n_config cvar" % config)
        elif z3n_config == "0":
            self.console.write('load "%s"' % (config))
            self.console.write('z3n_config ""')
            client.message("Config file ^4%s^7 loaded and removed as z3n_config cvar" % config)
        else:
            client.message('Invalid syntax, use ^2!help ^4load')
            return False
